...
#
class MapotempoFleet < DeviceBase
  def initialize
    ActiveSupport::Cache::FileStore.new(File.join(Dir.tmpdir, 'mapotempo_fleet'), namespace: 'mapotempo_fleet', expires_in: 60*60*24)
  end

  def definition
    {
      device: 'mapotempo_fleet',
      label: 'Mapotempo Fleet',
      label_small: 'Mapotempo',
  ...
  def fetch_stops(customer, date)
    ...
    planning.routes.select(&:vehicle_usage).flat_map{ |r|
      if r.last_sent_at && r.last_sent_at == Time.now.at_midnight
        started = false
